...
# think of a module as a bucket with a bunch of reusable code -- More about module later
...
# print(alphabet.pop(2)) # removes element in the back of list & returns the removed item.
#del alphabet[0:3]
...
# W/O list() around map(), it results in map object which is an iterable - This would require the use of for-in loop which is a basic way to code. To iterate over the map object, use list()
...
# This results in a list of booleans for each value in the tuple from the list, indicating if each value meets the expression or not
prices_filtered = [product[1] >= 110 for product in products]
...
# Removes the last element (one in the back) of the list
