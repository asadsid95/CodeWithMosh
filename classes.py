...
class MyPoint1:
    ...
    def draw(self):
        print(
            f"value of 'first' attribute: {self.first}. \nvalue of 'second'attribute: {self.second}.")
        return 'end of draw()'
...
class MyPoint3:
    ...
    # This is a decorator which extends the behaviour of a method or a function (its functionality to be explored shortly)
    @classmethod
...
# .zero() is considered a factory method. It creates new objects
point1 = MyPoint3.zero()
# print(point1)
...
class TagCloud:
    ...
    def add(self, tag):
        ...
        # .get() is for dict's method to return value of the tag 'key'. If the value isn't present, then create the keys and set to default value provided (0)
    ...
    def add_case_sensitive(self, tag):
        # Try removing .upper() from get(), and see what happens; think of why it has to be stated on both sides
...
# 10. In 8., we could only read the count of a tag. How can we set it?
cloud['java'] = 10
...
class Product:
    ...
    # 4. This takes the value, checks if its negative (if so, ValueError gets raised)
    ...
    # Note the functions in parameter are being passed as REFERENCE, not called. This property function will return property object which will use the functions from the parameter to read the value of the attribute (price)
    price = property(get_price, set_price)


# 1. But... price can not be negative so how do we prevent this?
product = Product(1050)
# print(product.price)
